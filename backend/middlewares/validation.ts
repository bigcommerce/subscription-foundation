...
export const validateBodyMiddleware = (
  ...
  return (req, res, next): any => {
    if (isNullOrUndefined(req?.body)) {
      console.error("request body undefined");
      req.body = {};
    }
    const bodyData = plainToClass(type, req?.body ?? {}) as typeof type;
...
export const validateParamMiddleware = (
  ...
  return (req, res, next): any => {
    const paramsData = plainToClass(type, req?.params ?? {}) as typeof type;
...
export const validateQueryMiddleware = (
  ...
  return (req, res, next): any => {
    const queryData = plainToClass(type, req?.query ?? {}) as typeof type;
